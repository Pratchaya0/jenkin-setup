...
pipeline {
    ...
    stages { 
        stage('Initialize Pipeline') {
            ...
            steps {  
                script {
                    updateGitHubStatus('pending', 'Build started', 'Initializing multi-server .NET deployment pipeline...')
                    
                    logSection("PIPELINE INITIALIZATION")
                    logInfo("Build Server", env.BUILD_AGENT_LABEL)
                    logInfo("Deploy Server", env.DEPLOY_AGENT_LABEL)
                    logInfo("Configuration", env.CONFIGURATION)
                ...
                script {
                    ...
                    powershell '''
                        $RequiredSettings = @(
                            ...
                            "BASE_BACKUPS_PATH", "CONFIGURATION", "PUBLISH_PATH",
                    ...
                    configFileProvider([configFile(fileId: "${env.CONFIG_FILE_ID}", variable: 'configFile')]) {
                        ...
                        try {
                            ...
                            config.eachWithIndex { project, index ->
                                logInfo("Project [${index}]", "${project.project_name} (${project.dotnet_version})")
                            }
                ...
                script {
                    ...
                    logInfo("Processing project", project.project_name)
                    
                    if (project.is_run_build) {
                        ...
                        if (validationErrors.size() > 0) {
                            ...
                            updateGitHubStatus('error', 'Configuration Error', "Missing required attributes for .NET project")
                            error("Missing required attributes for .NET project: ${validationErrors.join(', ')}")
                        }
                    }
                    ...
                    logInfo("Project Name", project.project_name)
                    logInfo(".NET Version", project.dotnet_version)
                    logInfo("Build Configuration", env.CONFIGURATION)
        ...
        stage('Build .NET Application') {
            ...
            steps {
                script {
                    updateGitHubStatus('pending', 'Building', 'Building .NET application on build server...')
                    
                    logSection("APPLICATION BUILD PROCESS")
                    
                    def config = readJSON text: env.CONFIG
                    def project = config[0]
                    
                    logInfo("Building project", project.project_name)
                    logInfo(".NET version", project.dotnet_version)
                    logInfo("Configuration", env.CONFIGURATION)
                    
                    if (project.is_run_build) {
                        try {
                            dir(project.project_name) {
                                def buildPath = "${env.WORKSPACE}\\${env.PUBLISH_PATH}\\${project.project_name}"
                                
                                withDotNet(sdk: project.dotnet_version) {
                                    logSubSection("Dependency Restoration")
                                    logInfo("Restore command", "dotnet restore")
                                    bat "dotnet restore"
                                    logSuccess("Dependencies restored successfully")
                                    
                                    logSubSection("Application Compilation")
                                    logInfo("Build command", "dotnet build --configuration ${env.CONFIGURATION} --no-restore")
                                    bat "dotnet build --configuration ${env.CONFIGURATION} --no-restore"
                                    logSuccess("Application compiled successfully")
                                    
                                    logSubSection("Publication for Deployment")
                                    logInfo("Publish command", "dotnet publish --configuration ${env.CONFIGURATION} --no-build")
                                    logInfo("Output directory", buildPath)
                                    bat "dotnet publish --configuration ${env.CONFIGURATION} --no-build --output \"${buildPath}\""
                                    logSuccess("Application published successfully")
                                }
                                
                                // Store build path for artifact archiving
                                env."BUILD_PATH_${project.project_name}" = buildPath
                                logSuccess("Build completed for ${project.project_name}")
                                logInfo("Artifacts location", buildPath)
                            }
                        } catch (Exception e) {
                            logError("Build failed: ${e.message}")
                            updateGitHubStatus('error', 'Build Failed', "Build failed for project: ${project.project_name}")
                            throw e
                        }
                    } else {
                        logInfo("Build status", "Skipped (not enabled for this project)")
                    }
                    
                    updateGitHubStatus('pending', 'Build Successful', 'Build completed, preparing for testing...')
                }
            }
            ...
            post {
                success {
                    script {
                        ...
                        updateGitHubStatus('pending', 'Artifacts Ready', 'Build artifacts ready for testing and deployment...')
                    }
                }
            }
        }
        ...
        stage('Run Unit Tests') {
            ...
            steps {
                script {
                    ...
                    if (project.is_run_test && project.project_test_name) {
                        updateGitHubStatus('pending', 'Testing', 'Running unit tests on build server...')
                        
                        logSection("UNIT TESTING")
                        logInfo("Test project", project.project_test_name)
                        logInfo(".NET version", project.dotnet_version)
                        
                        try {
                            dir(project.project_test_name) {
                                withDotNet(sdk: project.dotnet_version) {
                                    logSubSection("Test Execution")
                                    logInfo("Test command", "dotnet test --no-build --verbosity normal")
                                    bat "dotnet test --no-build --verbosity normal"
                                    logSuccess("All tests passed successfully")
                                }
                            }
                            updateGitHubStatus('pending', 'Tests Passed', 'All tests passed, proceeding to deployment...')
                        } catch (Exception e) {
                            logError("Tests failed: ${e.message}")
                            updateGitHubStatus('failure', 'Tests Failed', "Tests failed for project: ${project.project_name}")
                            throw e
                        }
                    } else {
                        logSection("UNIT TESTING")
                        logInfo("Test status", "Skipped (not enabled or no test project specified)")
                    }
                }
            }
        }
            ...
            steps {
                script {
                    trackDeployment(env.DEPLOYMENT_ENVIRONMENT) {
                        updateGitHubStatus('pending', 'Deploying', 'Deploying application to target environment...')
                        
                        logSection("APPLICATION DEPLOYMENT")
                        logInfo("Deploy server", env.DEPLOY_AGENT_LABEL)
                        ...
                        if (project.is_run_build) {
                            try {
                                ...
                                def backupPath = "${backupsPath}\\${env.BUILD_NUMBER}_${project.iis_website_name}_${env.GITHUB_BRANCH_NAME}_${new Date().format('yyyy-MM-dd_HHmmss')}"
                                def buildPath = "${env.WORKSPACE}\\${env.PUBLISH_PATH}\\${project.project_name}"
                                ...
                                logSubSection("Deploying Application")
                                powershell """
                                    # Remove old files (except appsettings)
                                    if (Test-Path "${websitePath}") {
                                        \$ExistingFiles = Get-ChildItem -Path "${websitePath}\\*" -Exclude "appsettings*.json" -File -ErrorAction SilentlyContinue
                                        if (\$ExistingFiles) {
                                            \$ExistingFiles | Remove-Item -Force
                                            Write-Host "[INFO] Removed existing files (preserved appsettings)" -ForegroundColor Cyan
                                        }
                                    }
                                    
                                    # Deploy new files with config preservation
                                    if (Test-Path "${buildPath}") {
                                        \$AppSettingsFiles = Get-ChildItem -Path "${websitePath}\\appsettings*.json" -File -ErrorAction SilentlyContinue
                                        
                                        if (\$AppSettingsFiles) {
                                            # Exclude existing appsettings
                                            Get-ChildItem -Path "${buildPath}\\*" -Exclude "appsettings*.json" | Copy-Item -Destination "${websitePath}" -Recurse -Force
                                            Write-Host "[INFO] Deployed files with preserved appsettings" -ForegroundColor Cyan
                                        } else {
                                            # Copy all files
                                            Copy-Item -Path "${buildPath}\\*" -Destination "${websitePath}" -Recurse -Force
                                            Write-Host "[INFO] Deployed all application files" -ForegroundColor Cyan
                                        }
                                        
                                        \$DeployedFiles = Get-ChildItem -Path "${websitePath}" -File -Recurse
                                        Write-Host "[SUCCESS] Deployed \$(\$DeployedFiles.Count) files to target directory" -ForegroundColor Green
                                ...
                                logSuccess("Deployment completed successfully for ${project.project_name}")
                                
                            } catch (Exception e) {
                                logError("Deployment failed: ${e.message}")
                                updateGitHubStatus('failure', 'Deployment Failed', "Deployment failed for project: ${project.project_name}")
                                throw e
                            }
                        } else {
                            logInfo("Deployment", "Skipped (build not enabled for this project)")
                        }
                        
                        updateGitHubStatus('pending', 'Deployment Successful', 'Application deployed successfully, running cleanup...')
                    }
                }
            }
        }
        ...
        stage('Cleanup and Finalize') {
            ...
            steps {
                script {
                    ...
                    if (project.is_run_build && project.is_cleanup) {
                        logSubSection("Backup Cleanup")
                        logInfo("Project", project.project_name)
                    ...
                    updateGitHubStatus('success', 'Pipeline Completed', 'Multi-server .NET deployment completed successfully!')
                }
            }
        }
        ...
        stage('Deployment Summary') {
            ...
            steps {
                script {
                    ...
                    logInfo(".NET Version", project.dotnet_version)
                    logInfo("Build Configuration", env.CONFIGURATION)
                    logInfo("Test Project", project.project_test_name ?: "None")
                    ...
                    logSuccess("✓ .NET SDK version management")
                    logSuccess("✓ Unit testing integration")
                    logSuccess("✓ IIS lifecycle management")
                    logSuccess("✓ Configuration file preservation (appsettings)")
                    ...
                    echo "1. Verify deployed application functionality"
                    echo "2. Check IIS website and application pool status"
                    echo "3. Review backup files on deploy server"
                    echo "4. Test API endpoints and application features"
                    echo "5. Monitor application performance and logs"
                    
                    logSuccess("Multi-server .NET deployment pipeline completed successfully!")
                }
            }
        }
    }
    ...
    post {
        ...
        success {
            script {
                updateGitHubStatus('success', 'Pipeline Success', 'Multi-server .NET deployment completed successfully!')
                
                logSection("PIPELINE SUCCESS")
                logSuccess("Multi-server .NET deployment completed successfully!")
                logInfo("Status", "All stages completed without errors")
                logInfo("Deployment", "Application is now live on target environment")
        ...
        failure {
            script {
                updateGitHubStatus('failure', 'Pipeline Failed', 'Multi-server .NET pipeline failed. Check Jenkins logs.')
                
                logSection("PIPELINE FAILURE")
                logError("Multi-server .NET deployment failed")
                logWarning("Check the following for troubleshooting:")
                echo "• Build server logs for .NET build failures"
                echo "• .NET SDK version compatibility"
                echo "• NuGet package restoration process"
                echo "• Unit test execution and results"
        ...
        aborted {
            script {
                updateGitHubStatus('error', 'Build Aborted', 'Multi-server .NET pipeline was aborted.')
...
def updateGitHubStatus(String state, String description, String context_description) {
    ...
    try {
        ...
        withCredentials([usernamePassword(
            ...
            headersMap.put('User-Agent', 'Jenkins-Multi-Server-DotNet/2.0')
...
def trackDeployment(String environment, Closure deploymentSteps) {
    ...
    try {
        ...
        // Mark as successful
        if (deploymentId) {
            updateDeploymentStatus(deploymentId, 'success', "Successfully deployed to ${environment}")
        }
